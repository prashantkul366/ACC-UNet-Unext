...
class SimpleTokenMLP(nn.Module):
    """
    Simple token MLP: input/output (B, N, C)
    Replaces KAN-based FKANMLP where you want a standard MLP.
    """
    def __init__(self, dim, mlp_dim=None, dropout=0.1):
        super().__init__()
        mlp_dim = mlp_dim or int(dim * 4)
        self.norm = nn.LayerNorm(dim)
        self.fc1 = nn.Linear(dim, mlp_dim)
        self.act = nn.GELU()
        self.fc2 = nn.Linear(mlp_dim, dim)
        self.dropout = nn.Dropout(dropout)
        print("Using SimpleTokenMLP instead of FKANMLP")
...
#         self.ffn1 = SimpleTokenMLP(dim, mlp_dim)
...
#         self.ffn2 = SimpleTokenMLP(dim, mlp_dim)
...
class FinalKANRefine3D(nn.Module):
    ...
    def __init__(self, in_channels: int, mlp_ratio: float = 4.0):
        super().__init__()
        print("Initializing final MLP refinement block")
        mlp_dim = int(in_channels * mlp_ratio)
        # self.kan_mlp = FKANMLP(dim=in_channels, mlp_dim=mlp_dim)
        self.kan_mlp = SimpleTokenMLP(dim=in_channels, mlp_dim=mlp_dim)
...
class SegMamba(nn.Module):
    def __init__(
        ...
        print("Initializing SegMamba with Hybrid Encoder - GSC + MDTA + MambaVisionMixer + MLP-Refine with Deep Supervision")
