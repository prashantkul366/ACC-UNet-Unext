...
class DualCrossAttentionFusion(nn.Module):
    ...
    1) Image attends to Text   (Image ← Text)
    2) Text attends to Image   (Text ← Image)

    Returns:
        fused_image: (B,C,D,H,W)
        updated_text: (B,T,768)
    """
    ...
    def __init__(self, img_dim, text_dim=768, num_heads=4):
        ...
        # -----------------------------
        self.norm_img1 = nn.LayerNorm(img_dim)
        self.norm_txt1 = nn.LayerNorm(text_dim)

        self.q_img = nn.Linear(img_dim, img_dim)
        self.k_txt = nn.Linear(text_dim, img_dim)
        self.v_txt = nn.Linear(text_dim, img_dim)

        self.attn_img_to_txt = nn.MultiheadAttention(
            embed_dim=img_dim,
            num_heads=num_heads,
            batch_first=True,
        )

        self.out_img = nn.Linear(img_dim, img_dim)

        # -----------------------------
        # Text ← Image Attention
        # -----------------------------
        self.norm_txt2 = nn.LayerNorm(text_dim)
        self.norm_img2 = nn.LayerNorm(img_dim)

        self.q_txt = nn.Linear(text_dim, text_dim)
        self.k_img = nn.Linear(img_dim, text_dim)
        self.v_img = nn.Linear(img_dim, text_dim)

        self.attn_txt_to_img = nn.MultiheadAttention(
            embed_dim=text_dim,
            num_heads=num_heads,
            batch_first=True,
        )

        self.out_txt = nn.Linear(text_dim, text_dim)
    ...
    def forward(self, x5d, text_tokens):
        ...
        # ============================================
        img_tokens = x5d.view(B, C, N).transpose(1, 2)  # (B,N,C)

        # ======================================================
        # (1) IMAGE attends to TEXT  → Image gets enriched
        # ======================================================
        img_norm = self.norm_img1(img_tokens)
        txt_norm = self.norm_txt1(text_tokens)

        Q_img = self.q_img(img_norm)
        K_txt = self.k_txt(txt_norm)
        V_txt = self.v_txt(txt_norm)

        img_out, _ = self.attn_img_to_txt(Q_img, K_txt, V_txt)

        # Residual update
        img_tokens = img_tokens + self.out_img(img_out)

        # ======================================================
        # (2) TEXT attends to IMAGE  → Text gets refined
        # ======================================================
        txt_norm2 = self.norm_txt2(text_tokens)
        img_norm2 = self.norm_img2(img_tokens)

        Q_txt = self.q_txt(txt_norm2)
        K_img = self.k_img(img_norm2)
        V_img = self.v_img(img_norm2)

        txt_out, _ = self.attn_txt_to_img(Q_txt, K_img, V_img)

        # Residual update
        text_tokens = text_tokens + self.out_txt(txt_out)

        # ============================================
        # Reshape Image back → 5D Feature Map
        # ============================================
        fused_img = img_tokens.transpose(1, 2).view(B, C, D, H, W)

        return fused_img, text_tokens
...
class DualCrossAttention(nn.Module):
    """
    Dual Cross Attention:
    1) Image attends to Text
    2) Text attends to Image
    """

    def __init__(self, dim, text_dim=768, num_heads=4):
        super().__init__()

        # project text → image dim
        self.text_proj = nn.Linear(text_dim, dim)

        # Image ← Text
        self.img_to_txt = HSLCA(dim=dim, num_heads=num_heads)

        # Text ← Image
        self.txt_to_img = HSLCA(dim=dim, num_heads=num_heads)
    ...
    def forward(self, x5d, text_tokens):
        ...
        # ---- flatten image ----
        img_tokens = x5d.view(B, C, N).transpose(1, 2)

        # ---- project text ----
        txt_tokens = self.text_proj(text_tokens)

        # normalize
        img_tokens = self.norm_img(img_tokens)
        txt_tokens = self.norm_txt(txt_tokens)

        # ------------------------------
        # (1) Image attends to Text
        # ------------------------------
        img_tokens = self.img_to_txt(img_tokens, txt_tokens)

        # ------------------------------
        # (2) Text attends to Image
        # ------------------------------
        txt_tokens = self.txt_to_img(txt_tokens, img_tokens)

        # ---- reshape image back ----
        fused_img = img_tokens.transpose(1, 2).view(B, C, D, H, W)

        return fused_img, txt_tokens
