...
class SpatialStateFusion(nn.Module):
    def __init__(self, dim):
        ...
        self.dw5 = nn.Conv2d(dim, dim, 3, padding=5, dilation=5, groups=dim)
        self.alpha = nn.Parameter(torch.ones(3))

    def forward(self, x):
        # x: (B, C, H, W)
        return (
            self.alpha[0] * self.dw1(x)
          + self.alpha[1] * self.dw3(x)
          + self.alpha[2] * self.dw5(x)
        )
...
class SegMamba(nn.Module):
    def __init__(
        ...
        print("GSC + MDTA + MambaVisionMixer with SPATIAL MAMBA + KAN-Refine with Deep Supervision")
