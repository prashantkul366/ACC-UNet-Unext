...
class UNext_CMRF_Dense_Skip(nn.Module):
    ...
    def __init__(self, n_channels=3, n_classes=1,  deep_supervision=False,img_size=224, patch_size=16, in_chans=3,  embed_dims=[ 128, 160, 256],
        ...
        print("UNext CMRF Encoders with Dense Skip Initiated")
        ...
        self.block2 = nn.ModuleList([shiftedBlock(
            ...
            sr_ratio=sr_ratios[0])])
                ...
                # ---- UNet++ dense-skip heads (H_i_j) ----
        c0, c1, c2, c3 = 16, 32, 128, 160

        # j = 1
        self.H0_1 = ConvBNReLU(in_ch=c0 + c1, out_ch=c0)     # [X_0,0, Up(X_1,0)]
        self.H1_1 = ConvBNReLU(in_ch=c1 + c2, out_ch=c1)     # [X_1,0, Up(X_2,0)]
        self.H2_1 = ConvBNReLU(in_ch=c2 + c3, out_ch=c2)     # [X_2,0, Up(X_3,0)]

        # j = 2
        self.H0_2 = ConvBNReLU(in_ch=c0 + c0 + c1, out_ch=c0)  # [X_0,0, X_0,1, Up(X_1,1)]
        self.H1_2 = ConvBNReLU(in_ch=c1 + c1 + c2, out_ch=c1)  # [X_1,0, X_1,1, Up(X_2,1)]

        # j = 3
        self.H0_3 = ConvBNReLU(in_ch=c0 + c0 + c0 + c1, out_ch=c0)  # [X_0,0, X_0,1, X_0,2, Up(X_1,2)]

        # (Optional) deep supervision heads
        self.ds01 = nn.Conv2d(c0, n_classes, 1)
        self.ds02 = nn.Conv2d(c0, n_classes, 1)
        self.ds03 = nn.Conv2d(c0, n_classes, 1)
    ...
    def forward(self, x):
        ...
        # Map enc features to UNet++ base nodes
        X00, X10, X20, X30 = t1, t2, t3, t4  # /2, /4, /8, /16

        # j = 1
        X01 = self.H0_1(torch.cat([X00, up_to(X10, X00)], dim=1))
        X11 = self.H1_1(torch.cat([X10, up_to(X20, X10)], dim=1))
        X21 = self.H2_1(torch.cat([X20, up_to(X30, X20)], dim=1))

        # j = 2
        X02 = self.H0_2(torch.cat([X00, X01, up_to(X11, X00)], dim=1))
        X12 = self.H1_2(torch.cat([X10, X11, up_to(X21, X10)], dim=1))

        # j = 3
        X03 = self.H0_3(torch.cat([X00, X01, X02, up_to(X12, X00)], dim=1))
...
if __name__ == '__main__':
    ...
    model = UNext_CMRF_Dense_Skip(num_classes=1, input_channels=3)
    model.eval()
