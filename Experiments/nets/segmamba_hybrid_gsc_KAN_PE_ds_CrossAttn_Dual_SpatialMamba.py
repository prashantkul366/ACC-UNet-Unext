...
class SpatialMambaSliceBlock(nn.Module):
    """
    Apply Spatial-Mamba block slice-wise on (H,W) for each depth slice.
    Input:  (B,C,D,H,W)
    Output: (B,C,D,H,W)
    """
    ...
    def forward(self, x5d):
        ...
        # reshape → treat each slice as batch element
        x2d = x5d.permute(0, 2, 3, 4, 1).contiguous()
        # (B,D,H,W,C)

        x2d = x2d.view(B * D, H, W, C)
        ...
        # reshape back
        x2d = x2d.view(B, D, H, W, C)
        x_out = x2d.permute(0, 4, 1, 2, 3).contiguous()
...
class MambaEncoder(nn.Module):
    def __init__(
        ...
        for i in range(4):
            ...
            stage = nn.Sequential(
                *[
                    SpatialMambaSliceBlock(dim=dims[i])
                    for _ in range(depths[i])
                ]
            )
