...
class ConvKANFFN2D(nn.Module):
    """
    FFN block for tokens (B, N, C) that does:

      conv3x3 → conv3x3 → LayerNorm
      → KAN → dwconv3x3 → KAN → dwconv3x3
      → + residual (from after conv3x3+LN)

    Assumes N = H * W and H == W (square spatial map).
    """
    def __init__(self, dim, mlp_dim):
        ...
        # LayerNorm after the two conv3x3 (on tokens)
        self.post_conv_ln = nn.LayerNorm(dim)
        ...
        self.act = nn.GELU()
        print("Using Efficient kan block :  ConvKANFFN2D MLP block")

    def _tokens_to_2d(self, x):
        # x: (B, N, C)
        B, N, C = x.shape
        H = W = int(math.sqrt(N))
        assert H * W == N, f"ConvKANFFN2D expects N=H*W, got N={N}"
        x_2d = x.permute(0, 2, 1).reshape(B, C, H, W)  # (B, C, H, W)
        return x_2d, H, W

    def _2d_to_tokens(self, x_2d):
        # x_2d: (B, C, H, W) -> (B, N, C)
        B, C, H, W = x_2d.shape
        N = H * W
        x = x_2d.reshape(B, C, N).permute(0, 2, 1)
        return x

    def forward(self, x):
        """
        x: (B, N, C)
        NOTE: this is already LN'ed once by ln4 outside,
              and we add another LN internally after convs.
        """
        B, N, C = x.shape

        # ---- conv3x3 ×2 in spatial domain ----
        x_2d, H, W = self._tokens_to_2d(x)      # (B, C, H, W)
        x_2d = self.act(self.conv1(x_2d))
        x_2d = self.act(self.conv2(x_2d))

        # back to tokens
        x_conv = self._2d_to_tokens(x_2d)       # (B, N, C)

        # ---- LayerNorm after conv3x3 ----
        x_conv = self.post_conv_ln(x_conv)      # (B, N, C)

        # this is the residual base: from AFTER conv3x3+LN
        res_base = x_conv

        # ---- KAN 1 ----
        x_flat = x_conv.reshape(B * N, C)
        x_flat = self.kan1(x_flat)
        x = x_flat.view(B, N, C)

        # ---- depthwise conv 3x3 (1st) ----
        x_2d, _, _ = self._tokens_to_2d(x)
        x_2d = self.act(self.dwconv1(x_2d))
        x = self._2d_to_tokens(x_2d)

        # ---- KAN 2 ----
        x_flat = x.reshape(B * N, C)
        x_flat = self.kan2(x_flat)
        x = x_flat.view(B, N, C)

        # ---- depthwise conv 3x3 (2nd) ----
        x_2d, _, _ = self._tokens_to_2d(x)
        x_2d = self.act(self.dwconv2(x_2d))
        x = self._2d_to_tokens(x_2d)

        # ---- internal residual: (after conv3x3+LN) -> (after 2nd dwconv) ----
        x = x + res_base
...
class TransformerMambaBlock(nn.Module):
    def __init__(self, dim, num_heads=4, mlp_ratio=4.0,
        ...
        self.ffn2 = ConvKANFFN2D(dim, mlp_dim)
...
class SegMamba(nn.Module):
    def __init__(
        ...
        print("Initializing SegMamba with - GSC + MDTA + MambaVisionMixer + KAN-Refine + Efficient KAN Block")
